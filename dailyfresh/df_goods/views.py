...
def detail(request):
	...
	if goods_ids!='':#判断是否有浏览记录，如果有则继续判断
		...
		goods_ids1.insert(0,good_id)#添加到第一个
		if len(good_ids1)>=6:#如果超过6个则删除最后一个
			del goods_ids1[5]
...
class MySearchView(SearchView):#定义一个类，继承于searchview
	def extra_context(self):
		context=super(MySearchView,self).extra_context()#调用父级中的一个方法建立对象
		context['title']='搜索'   #新增自定义上下文
